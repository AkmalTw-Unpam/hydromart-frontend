...
const CustomTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div className="bg-white dark:bg-navy-800 border border-slate-100 dark:border-navy-700 rounded-xl px-3 py-2 text-xs shadow-xl">
      <p className="font-semibold text-slate-700 dark:text-slate-200 mb-1">{label || 'Tanpa Tanggal'}</p>
      {payload.map(p => (
        <p key={p.name} style={{ color: p.color }}>{p.name}: <strong>{p.value || 0}</strong></p>
      ))}
    </div>
  )
}
          ...
          {loading ? <Skeleton className="h-48 m-4 rounded-lg" /> : (
            <div className="divide-y divide-slate-100 dark:divide-navy-700/40">
              {(data?.low_stock_items || []).length === 0 ? (
                ...
                <div key={item.id || Math.random()} className="flex items-center justify-between px-5 py-3.5 hover:bg-slate-50/50 dark:hover:bg-navy-800/30 transition-colors">
                  <div className="min-w-0 flex-1 pr-3">
                    <p className="text-xs font-semibold text-slate-800 dark:text-slate-200 truncate">{item?.name || 'Produk Tanpa Nama'}</p>
                    <p className="text-[10px] text-slate-400 font-mono mt-0.5">{item?.code || '-'}</p>
                  </div>
                  <div className="text-right flex-shrink-0">
                    <StockBadge status={item?.status || 'danger'} />
                    <p className="text-[10px] text-slate-400 mt-1 font-medium">
                      {item?.stock || 0}/{item?.min_stock || 0} {item?.unit || 'Unit'}
                    </p>
                  </div>
                </div>
              ))}
          ...
          {loading ? <Skeleton className="h-48 m-4 rounded-lg" /> : (
            <div className="divide-y divide-slate-100 dark:divide-navy-700/40 max-h-72 overflow-y-auto">
              {(data?.recent_movements || []).length === 0 ? (
                ...
                <div key={m.id || Math.random()} className="flex items-center gap-3 px-5 py-3.5 hover:bg-slate-50/50 dark:hover:bg-navy-800/30 transition-colors">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-semibold text-slate-800 dark:text-slate-200 truncate">
                      {m?.item?.name || 'Nama barang tidak tersedia'}
                    </p>
                    <p className="text-[10px] text-slate-400 mt-0.5 font-medium">
                      <span className="text-sky-500 dark:text-cyan-400">{m?.user?.name || 'Sistem'}</span> · {m?.quantity || 0} {m?.item?.unit || 'Unit'}
                    </p>
                  </div>
